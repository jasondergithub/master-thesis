...
class GNN(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.score_function1 = nn.Linear(opt["hidden_dim"], opt["hidden_dim"])
        # self.score_function1_5 = nn.Linear(opt["hidden_dim"], int(opt["hidden_dim"] / 2))
        self.score_function2 = nn.Linear(32, 10)
        self.score_function3 = nn.Linear(10,1)
        # self.view_function0 = nn.Linear(opt["hidden_dim"] * 2, opt["hidden_dim"]) # global append sub_global + vector
        self.view_function1 = nn.Linear(opt["hidden_dim"], 32) # sub_global + vector
        # self.view_function1 = nn.Linear(opt["hidden_dim"] * 2, 32)
        # self.view_function1 = nn.Bilinear(opt["hidden_dim"], opt["hidden_dim"], 32)
